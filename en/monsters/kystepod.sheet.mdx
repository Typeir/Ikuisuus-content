...
The Lunar Kystepod is biologically tethered to Cerithol Lunami. While within [= 60 stride =] of a Cerithol, it gains **pack tactics** and **cannot be frightened**. Additionally, the Cerithol can communicate telepathically with it.
...
- **Chitin Claws**
  ...
  _Hit:_ 12 ([% 2d6 +4 slashing %]). If the target is a creature, it must succeed on a DC 12 Strength saving throw or be knocked prone.

- **Acidic Ventral Harpoon**
  _Melee Weapon Attack:_ +5 to hit, reach [= 2 stride =], one target.  
  _Hit:_ 14 ([% 2d8 +4 piercing %]) plus 7 ([% 2d6 chemical %]). If the target is already prone, it suffers **maximum chemical damage**.

- **Tendril**
  _Melee Weapon Attack:_ +5 to hit, reach [= 2 stride =], one target.  
  _Hit:_ 16 ([% 2d10 +4 piercing %]).
