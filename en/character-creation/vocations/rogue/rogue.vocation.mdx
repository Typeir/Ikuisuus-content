---
source: Ikuisuus
contentType: vocations
grants:
  'Expertise':
    - 'skill:any:expertise:2'
---

# Rogue

Rogues rely on cunning, stealth, and their foes' vulnerabilities to get the upper hand in any situation. They have a knack for finding the solution to just about any problem. A few even learn magical tricks to supplement their other abilities. Many Rogues focus on stealth and deception, while others refine skills that help them in a dungeon environment, such as climbing, finding and disarming traps and opening locks.

In combat, Rogues prioritize subtle strikes over brute strength. They would rather make one precise strike than wear an opponent down with a barrage of blows.

Some Rogues began their careers as criminals, while others used their cunning to fight crime. Whatever a Rogue's relation to the law, no common criminal or officer of the law can match the subtle brilliance of the greatest Rogues.

---

## Core Rogue Traits

| Trait                          | Value                                                                                                                                                          |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Primary Ability**            | Dexterity                                                                                                                                                      |
| **Hit Point Die**              | d8 per Rogue level                                                                                                                                             |
| **Saving Throw Proficiencies** | Dexterity and Intelligence                                                                                                                                     |
| **Skill Proficiencies**        | Choose 4: Acrobatics, Athletics, Deception, Insight, Intimidation, Investigation, Perception, Persuasion, Sleight of Hand, or Stealth                          |
| **Weapon Proficiencies**       | Simple weapons and Martial weapons with Finesse or Light                                                                                                       |
| **Trade Proficiencies**        | [Thievery](/en/library/items/tools/thievery)                                                                                                                   |
| **Armor Training**             | Light armor                                                                                                                                                    |
| **Starting Equipment**         | (A) Leather Armor, 2 Daggers, Shortsword, Shortbow, 20 Arrows, Quiver, a [Thievery](/en/library/items/tools/thievery) kit, Burglar's Pack, 8 GP<br/>(B) 100 GP |

---

## Becoming a Rogue

### As a Level 1 Character

- Gain all traits listed in the **Core Rogue Traits** table.
- Gain the Rogue’s level 1 features.

---

### As a Mixed Character

- Gain the Rogue's **Hit Point Die**.
- Gain the Rogue's weapon proficiencies.
- Pick one skill proficiency from the Core Rogue Traits table.
- Gain the Rogue's level 1 features.

---

## Rogue Progression

| Level | Tier Bonus | Features                                               | Sneak Attack |
| ----- | ----------------- | ------------------------------------------------------ | ------------ |
| 1     | +1                | Expertise, Sneak Attack, Thieves' Cant, Weapon Mastery | [% 1d6 %]          |
| 2     | +1                | Cunning Action                                         | [% 1d6 %]          |
| 3     | +1                | Rogue Specialization, Steady Aim                       | [% 2d6 %]          |
| 4     | +2                | Feat                                                   | [% 2d6 %]          |
| 5     | +2                | Cunning Strike, Uncanny Dodge                          | [% 3d6 %]          |
| 6     | +2                | Expertise                                              | [% 3d6 %]          |
| 7     | +3                | Evasion, Reliable Talent                               | [% 4d6 %]          |
| 8     | +3                | Feat                                                   | [% 4d6 %]          |
| 9     | +3                | Specialization Feature                                 | [% 5d6 %]          |
| 10    | +4                | Feat                                                   | [% 5d6 %]          |
| 11    | +4                | Improved Cunning Strike                                | [% 6d6 %]          |
| 12    | +4                | Feat                                                   | [% 6d6 %]          |
| 13    | +5                | Specialization Feature                                 | [% 7d6 %]          |
| 14    | +5                | Devious Strikes                                        | [% 7d6 %]          |
| 15    | +5                | Slippery Mind                                          | [% 8d6 %]          |
| 16    | +6                | Feat                                                   | [% 8d6 %]          |
| 17    | +6                | Specialization Feature                                 | [% 9d6 %]          |
| 18    | +6                | Elusive                                                | [% 9d6 %]          |
| 19    | +7                | Epic Boon                                              | [% 10d6 %]         |
| 20    | +7                | Stroke of Luck                                         | [% 10d6 %]         |

---

<Collapsible>
## 1st Level – Expertise

At level 1, choose two skill proficiencies. You double your tier bonus with them.

At level 6, choose two more.

</Collapsible>

<Collapsible>
## 1st Level – Sneak Attack

Once per turn, when you:

- Have **Advantage** on the attack roll, **or**
- An ally is within [= 1 stride =] of the target and you don’t have Disadvantage,

You deal extra damage. The attack must use a **Finesse or Ranged weapon**. The damage type matches the weapon.

</Collapsible>

<Collapsible>
## 1st Level – Thieves' Cant

You know **Thieves' Cant** and one other language of your choice.

</Collapsible>

<Collapsible>
## 1st Level – Weapon Mastery

Choose **2 weapons** with which you can use mastery properties. You can change these choices after a Recovery.

</Collapsible>

<Collapsible>
## 2nd Level – Cunning Action

At level 2, take **Dash**, **Disengage**, or **Hide** as a **Minor Action**.

</Collapsible>

<Collapsible>
## 3rd Level – Steady Aim

At level 3, if you are **steady**, use a **Minor Action** to give yourself Advantage on your next attack this turn.

</Collapsible>

<Collapsible open>
## 3rd Level – Rogue Specialization

At level 3, choose one:

| Specialization                                                                                     |
| -------------------------------------------------------------------------------------------------- |
| [Arcane Trickster](/en/library/character-creation/vocations/rogue/arcane-trickster.specialization) |
| [Assassin](/en/library/character-creation/vocations/rogue/assassin.specialization)                 |
| [Sellsword](/en/library/character-creation/vocations/rogue/sellsword.specialization)               |
| **Soulknife**                                                                                      |
| [Thief](/en/library/character-creation/vocations/rogue/thief.specialization)                       |

</Collapsible>

<Collapsible>
## 4th Level – Ability Score Improvement

At levels 4, 8, 10, 12, and 16, take the [Ability Score Improvement feat](/en/library/character-creation/feats/ability-score-improvement) or another feat.

</Collapsible>

<Collapsible>
## 5th Level – Cunning Strike

At level 5, when you Sneak Attack, you may sacrifice dice to apply additional effects:

| Effect       | Cost | Description                                                                                                     |
| ------------ | ---- | --------------------------------------------------------------------------------------------------------------- |
| **Poison**   | [% 1d6 %]  | Con save or Poisoned (1 min). Save ends each turn. Requires [Poisoncraft](/en/library/items/tools/poisoncraft). |
| **Trip**     | [% 1d6 %]  | Dex save or target is Prone.                                                                                    |
| **Withdraw** | [% 1d6 %]  | Move up to half your speed without provoking Opportunity Attacks.                                               |

</Collapsible>

<Collapsible>
## 5th Level – Uncanny Dodge

At level 5, when a creature hits you, use your **Reaction** to halve the damage.

</Collapsible>

<Collapsible>
## 7th Level – Evasion

At level 7, when making a Dex save for half damage, take **no damage** on a success and **half** on a failure.

</Collapsible>

<Collapsible>
## 7th Level – Reliable Talent

At level 7, when making a check using a proficient skill or tool, treat rolls of **9 or lower as 10**.

</Collapsible>

<Collapsible>
## 11th Level – Improved Cunning Strike

At level 11, you may use **two Cunning Strike effects** per Sneak Attack.

</Collapsible>

<Collapsible>
## 14th Level – Devious Strikes

At level 14, new Cunning Strike options:

| Effect        | Cost | Description                                                                          |
| ------------- | ---- | ------------------------------------------------------------------------------------ |
| **Daze**      | [% 2d6 %]  | Con save or target may only move or act, not both, next turn.                        |
| **Knock Out** | [% 6d6 %]  | Con save or target falls Unconscious (1 min or until damaged). Save at end of turns. |
| **Obscure**   | [% 3d6 %]  | Dex save or target is Blinded until end of next turn.                                |

</Collapsible>

<Collapsible>
## 15th Level – Slippery Mind

At level 15, gain proficiency in **Wisdom and Charisma saving throws**.

</Collapsible>

<Collapsible>
## 18th Level – Elusive

At level 18, **no attack roll can have Advantage** against you unless you are Incapacitated.

</Collapsible>

<Collapsible>
## 19th Level – Epic Boon

At level 19, take an [Epic Boon feat](/en/library/character-creation/feats).

Recommended: [Boon of the Night Spirit](/en/library/character-creation/feats/boon-of-the-night-spirit)

</Collapsible>

<Collapsible>
## 20th Level – Stroke of Luck

At level 20, if you fail a d20 Test, treat it as a **20**. Use once per Repose or Recovery.

</Collapsible>
