...
- **Damage**: [% 2d6 slashing %]
- **Weight per sword**: [= 3 burden =]
- **Quiver weight**: [= 5 burden =]
...
- **Thunderous Hurl** _(1/Repose, or 3/Repose with mastery)_:  
  As an **attack**, you may hurl a greatsword from the quiver at a target within **[= 6 stride =]/[= 12 stride =]**:
  - All creatures in a **[= 1 stride =]-wide line** between you and the target must make a **Dexterity saving throw**  
    (DC 10 + Strength mod + tier bonus)
  - **On failure**: They take **[% 2d6 lightning %]**
  - **On hit**: Target takes an additional **[% 2d6 bludgeoning %]**, and their **speed drops to 0** until your next turn
...
- **Mighty Armament**:
  ...
  - Drawn blades lose **Heavy** and **Two-Handed**, but gain **Thrown ([= 6 stride =]/[= 12 stride =])** and **Brittle** (breaks on impact)
...
- **Proficiency Override**:
  ...
  If both wielders are within **[= 12 stride =]** of each other, they gain the following benefits:

  - **Ignore the Mighty Armament restriction** (Strength 20 no longer required).
  - **Greatswords stored in the quiver weigh [= 0 burden =]** for carrying capacity.
  ...
  ***
  ...
  - The épée flies toward the quiver wielder if within **[= 120 stride =]**, traveling at:
    - **[= 24 stride =] per round**, or
    - **[= 12 stride =] per round through solid terrain**.
  ...
  If the wielders are **more than [= 120 stride =] apart**, the survivor gains **1 exhaustion per day at dawn** until the bond is restored or severed.

  ### Retribution Mode

  While under **Fated Demise**, this weapon gains enhanced effects:

  - **Storm Of Blades**_(3/Repose)_
  ...
  - You cast **Call Lightning** (DC 10 + Strength modifier + tier bonus) as though using a **3rd-level spell slot**
  ...
  - The weapon becomes **supercharged** for 1 minute:
    - While Call Lightning is active, **drawn greatswords crackle**, dealing **+[% 1d6 lightning %]** on hit
