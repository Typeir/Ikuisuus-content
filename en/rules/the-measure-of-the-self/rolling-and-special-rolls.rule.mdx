...
> **Note on Modifiers**:  
> Modifiers are usually listed in the desired effect. Common examples include: +1 weapons, the spells [_Bless_](/en/library/spells/bless) or [_Bane_](/en/library/spells/bane), exhaustion penalties, or item bonuses. If in doubt, consult the specific feature or item.

---

## Passive Checks

A **passive check** represents the average result for a task done repeatedly, or the result the GM uses when they want to determine success without calling for a roll—for example, quietly determining whether characters notice a hidden creature.
...
Passive checks are most commonly used for **Wisdom (Perception)**. A character's passive Perception acts as a floor: the GM compares it against a hidden creature's Stealth check result instead of calling for an active roll each time.
